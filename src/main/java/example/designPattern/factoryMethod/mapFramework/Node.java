...
public class Node extends MapElement {
	...
	public void circle(final int r, int d0, int p, final MapElement n, final boolean clockwise) {
		int q = 500;
		final int sleep = p / q;
			...
			public void run() {
				while (true)
					for (double t = 0; t <= 2 * Math.PI; t += d) {
						double x0 = n.getX();
						double y0 = n.getY();
						int y = (int) (y0 + r1 * ((clockwise) ? Math.sin(t) : Math.cos(t)));
						int x = (int) (x0 + r2 * ((clockwise) ? Math.cos(t) : Math.sin(t)));
						setX(x);
						setY(y);

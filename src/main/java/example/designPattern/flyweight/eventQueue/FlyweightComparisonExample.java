...
public class FlyweightComparisonExample {
	...
	public FlyweightComparisonExample() {
		...
		tStart = new Date();
		for (EventSymbol element : eventList)
			if (element.isEqualsTo(compEvent1))
				evn1count++;
			else if (element.isEqualsTo(compEvent2))
				evn2count++;
			else
				evn3count++;
		tFinish = new Date();
		compTime = tFinish.getTime() - tStart.getTime();
		System.out.println("E1: " + evn1count + " E2: " + evn2count + " E3: " + evn3count);
		...
		for (EventSymbol element : eventList)
			if (element == compEvent1)
				evn1count++;
			else if (element == compEvent2)
				evn2count++;
			else
				evn3count++;
		tFinish = new Date();
